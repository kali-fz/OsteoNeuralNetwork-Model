...
| 25 | **P1** | **The test suite writes to the production database.** `backend.create_user` routes to live D1 whenever a community key is configured and ignores the `path` argument, so `pytest` on a credentialled machine creates real accounts. **3 `@example.com` accounts are already in production D1** | `src/backend.py:49–60`; 4 failures in `tests/test_auth_database.py` reproduce at clean HEAD; D1 count confirmed 2026-08-24 | Force tests onto the local backend (fixture clearing the community key); purge the test accounts from D1; add a regression test asserting hermeticity |
